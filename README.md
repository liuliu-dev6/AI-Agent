...
一个具备意图识别、工具调用（Tool Calling）、多模型网关的 AI Agent 运行平台。区别于普通聊天应用，它实现了完整的 **Agent 执行循环**（感知 → 意图路由 → 工具执行 → 再决策 → 回复）。
...
![设置界面](src/assets/sampels/Agent设置.png)
_设置界面，支持模型选择、流式响应开关、API Key 配置等参数调整，优化用户体验。_
